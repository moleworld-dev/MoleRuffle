...
/// 变换插值开关(MOLE_INTERP=1):逻辑仍锁 24fps,但渲染按 ~60fps 出帧,在两个逻辑帧之间
/// 插值每个显示对象的变换 → 显示更顺却不加速游戏。默认关(正常玩=24fps)。进程级缓存一次。
#[cfg(not(target_os = "ios"))]
fn interp_enabled() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        matches!(
            std::env::var("MOLE_INTERP").as_deref(),
            Ok("1") | Ok("true") | Ok("on")
        )
    })
}
...
/// 大加载预清阈值:navigator 报告有 .swf 请求(切场景/魔灵等小游戏)且余量低于此值时,
/// 抢在资源解码落地前催收+清池腾余量;余量充裕(>此值)则不打扰(force_gc 有一帧级停顿)。
#[cfg(target_os = "ios")]
const MEM_PRELOAD_TRIM_MB: u64 = 2500;
...
impl App {
    ...
    fn build_player(&self, window: Arc<Window>) -> (Arc<Mutex<Player>>, Arc<AtomicBool>, String) {
        ...
        let (renderer, render_api) = {
            ...
            // 分配器提示按平台:iOS 用 MemoryUsage(紧凑省内存,避 jetsam);桌面用 Performance
            // (更大块少次数的次分配,分配更快、运行时抖动更小,桌面内存不缺,换性能)。
            #[cfg(target_os = "ios")]
    ...
    /// 绘制当前帧:先同步真实绘制尺寸(旋转/布局变化可能不发干净 Resized),再 render。
    ...
    fn mem_hud_tick(&mut self, since: Duration) {
        ...
        // 自适应内存守卫:余量低 → ①force_gc 打通死场景释放链(AVM2 BitmapData 解码位图/
        //   MovieClip→Arc<SwfMovie>→movie_library→位图句柄/网格,全靠 GC 收走才释放,而默认
...
impl ApplicationHandler<UserEvent> for App {
    ...
    fn window_event(&mut self, el: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                // 桌面的游戏帧已改在 about_to_wait 里同轮直渲(省一次往返),这里主要兜 OS 重绘。
                self.redraw_now();
            }
        ...
        // 输入即请求重绘(桌面):上面任一窗口事件(移动/点击/滚轮/键盘/缩放)处理后,
        // 若引擎变脏,立即请求一次重绘,而不是等下一次 about_to_wait 的 tick(最坏 ~33ms@30fps 后)。
        // 让 hover 高亮/点击反馈在下一个 vblank 就上屏、更跟手。零额外 GPU:仅 needs_render 为真才请求;
        // RedrawRequested 渲染后 needs_render 已清零,不会自触发。iOS 走帧率锁 tick 兜底,无需此路径。
        #[cfg(not(target_os = "ios"))]
        if self.with_player(|p| p.needs_render()).unwrap_or(false) {
            if let Some(w) = &self.window {
                w.request_redraw();
            }
        }
    ...
    fn about_to_wait(&mut self, el: &ActiveEventLoop) {
        ...
        #[cfg(not(target_os = "ios"))]
        let interp = interp_enabled();

        // 延迟线 #1:桌面在 tick 后【同轮直渲】,不再 request_redraw 等下一次事件循环——
        //   省掉一次 run-loop 往返(输入→上屏更跟手)。iOS 保留 request_redraw→RedrawRequested(平台约束)。
        #[cfg(not(target_os = "ios"))]
        {
            if interp {
                self.redraw_now(); // 每轮 60fps 出帧
        ...
        // 设 MOLE_PERF=0 可静音。用外部 `ps -o rss=,%cpu= -p <pid>` 配合看进程级 CPU/RSS。
        #[cfg(not(target_os = "ios"))]
        {
            use std::sync::atomic::Ordering::Relaxed;
            let since = self.last_mem_check.elapsed();
            if since.as_millis() >= 2000 && std::env::var("MOLE_PERF").as_deref() != Ok("0") {
                self.last_mem_check = now;
                let dt = since.as_secs_f64();
                let dframes = self.frames.saturating_sub(self.last_hud_frames);
                self.last_hud_frames = self.frames;
                let fps = dframes as f64 / dt;
        ...
        // 插值模式改睡 ~16.67ms(60fps 出帧节奏);tick 内部累加器保证逻辑仍只 24fps 推进。
        #[cfg(not(target_os = "ios"))]
        let sleep = if interp {
        ...
        #[cfg(target_os = "ios")]
        let sleep = til;
        el.set_control_flow(ControlFlow::WaitUntil(now + sleep));
    }
}
...
/// 安卓入口:JVM/GameActivity 无原生 main(),android-activity 的 glue 从这里进。
/// winit 开 `android-game-activity` feature 后,GameActivity 的 C 入口会调用本 `android_main`。
...
fn android_main(app: winit::platform::android::activity::AndroidApp) {
    ...
    let event_loop = EventLoop::<UserEvent>::with_user_event()
        ...
        .build()
        .expect("android EventLoop build 失败");
    if let Err(e) = run(event_loop) {
        tracing::error!("安卓 run 退出: {e}");
    }
}
